...
# Create a user.
RESP=$(curl --silent --write-out %{http_code} -d"username=${USERNAME}" -d"firstName=${FIRSTNAME}" -d"lastName=${LASTNAME}" -d"displayName=${DISPLAYNAME}" -d"password=${PASSWORD}" -d"locale=${LOCALE}" -d"timezone=${TIMEZONE}" http://${TENANT_HOST}/api/user/create)
STATUS=${RESP: -3:3}
BODY=$(echo ${RESP:0:${#RESP} -3} | python -m json.tool | grep 'id');
output $STATUS 201 "Created user ${BODY}"